...
def SKLearn_test_step(X_train, y_train, X_test, y_test, lambda_, n_hidden_neurons, num_ite, alpha, baseLineAccuracy):    
    # Creamos y configuramos el clasificador.
    mlp_sk = MLPClassifier(
        hidden_layer_sizes=(n_hidden_neurons,), # Una capa oculta con n_hidden_neurons neuronas
        activation='logistic',                  # Funcion de activacion sigmoidal
        alpha=lambda_,                          # Parametro de regularizacion
        learning_rate_init = alpha,             # Tasa de aprendizaje
        max_iter=num_ite,                       # Numero de iteraciones
        random_state=42,                        # Misma semilla para inicializacion
        solver='sgd',                           # Optimizador sgd
        verbose=False                           # Mostrar progreso del entrenamiento en consola
    )
    
    # Entrenamos: SKLearn espera las etiquetas de clase.
    # y_train debe ser la version plana de las etiquetas de clase (e.g., [1, 5, 2, ...]).
    ...
    accuracy_sk = accuracy_score(y_test.ravel(), y_pred_sk)
    
    print(f"Accuracy of SKLearn for lambda = {(lambda_):1.5f} : {(accuracy_sk):1.5f} expected accuracy is aprox: {(baseLineAccuracy):1.5f}")