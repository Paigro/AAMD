...
public class MLAgent : MonoBehaviour
{
    ...
    public float[] RunFeedForward(float[] modelInput)
    {
        ...
        Debug.Log($"   Columnas eliminadas: {string.Join(",", indicesToRemove)}");

        // One-Hot Encoding
        float[] beforeOHE = (float[])modelInput.Clone();
        modelInput = oneHotEncoding.Transform(modelInput);
        Debug.Log($"   Expansión: {beforeOHE.Length} → {modelInput.Length}");

        // StandardScaler
        float[] beforeScaler = (float[])modelInput.Clone();
